      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          {["all","pendiente","asignada","en_proceso","completada"].map(s=>(
            <button key={s} onClick={()=>setFilter(s)} style={filter===s?{background:NV.blue}:{}}
...
function Equipment({user,data,setData}){
  ...
  const saveEquip=()=>{
    ...
    const updated=editTarget?equip.map(e=>e.id===editTarget.id?{...e,...form,hours:parseInt(form.hours)||0}:e):[...equip,{id:uid(),...form,hours:parseInt(form.hours)||0}];
                        ...
                        <td className="px-4 py-2.5">
                          {isEditingThis?(
                            ...
                            <div className="flex items-center gap-2 group">
                              ...
                              {isSup&&<button onClick={()=>setEditingHours({id:e.id,val:String(e.hours||0)})} className="opacity-0 group-hover:opacity-100 transition p-1 rounded hover:bg-blue-50"><Edit2 size={11} style={{color:NV.blue}}/></button>}
                            </div>
                          )}
                        </td>
                        {isSup&&(
                          <td className="px-4 py-2.5">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={()=>{setForm({...e,hours:String(e.hours)});setEditTarget(e);setShowForm(true);}} className="p-1.5 rounded-lg hover:bg-blue-50" style={{color:NV.blue}}><Edit2 size={13}/></button>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    <span className={`px-2 py-0.5 rounded-full border text-xs font-bold ${dl<=0?"text-red-700 bg-red-50 border-red-200":dl<=7?"text-red-700 bg-red-50 border-red-200":dl<=30?"text-amber-700 bg-amber-50 border-amber-200":"text-emerald-700 bg-emerald-50 border-emerald-200"}`}>
                      {dl<=0?"VENCIDO":`En ${dl}d`}
                    </span>
                  </div>
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  {user.role==="supervisor"&&<button onClick={()=>generateOT(p)} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg text-white font-medium" style={{background:NV.blue}}><Zap size={12}/>Generar OT</button>}
                </div>
              </div>
            </div>
          );
      ...
      {showMasivo&&(
        <Modal title="Plan Masivo — Múltiples equipos" onClose={()=>setShowMasivo(false)} wide>
          ...
          <div className="grid grid-cols-2 gap-5">
            <div>
              <p className="text-gray-700 font-semibold text-sm mb-2">Equipos ({selEquips.length})</p>
              <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
                {equip.map(e=>{const checked=selEquips.includes(e.id);return(
                  <label key={e.id} className={`flex items-center gap-2.5 p-2.5 rounded-lg border cursor-pointer transition ${checked?"border-blue-300":"bg-gray-50 border-gray-200"}`} style={checked?{background:NV.light}:{}}>
...
function Indicadores({data}){
  ...
  return(
    <div className="p-6 space-y-6">
      ...
      <div className={`${card} overflow-hidden`}>
        <div className="px-4 py-3 text-xs text-white font-semibold uppercase tracking-wider" style={{background:NV.navyMid}}>KPIs por Equipo</div>
        <table className="w-full text-sm">
          ...
          <tbody>{equip.map((e,i)=>{const m=calc(e.id);return(
            <tr key={e.id} className={`border-b border-gray-100 last:border-0 hover:bg-blue-50/30 ${i%2===0?"bg-white":"bg-gray-50/40"}`}>
              ...
              <td className="px-4 py-2.5 text-right text-gray-600 text-xs font-mono">{(e.hours||0).toLocaleString()}h</td>
              ...
              <td className="px-4 py-2.5 text-center">
                {m.disp!==null?<div className="flex flex-col items-center gap-1"><span className={`font-bold text-sm ${dC(m.disp)}`}>{fP(m.disp)}</span><div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden"><div className={`h-full rounded-full ${m.disp>=90?"bg-emerald-500":m.disp>=70?"bg-amber-400":"bg-red-500"}`} style={{width:`${Math.min(100,m.disp)}%`}}/></div></div>:<span className="text-gray-400 text-xs">Sin datos</span>}
              </td>
            </tr>
          );})}
          </tbody>
          ...
          <div key={r.id} className={`bg-white border rounded-xl p-5 shadow-sm ${r.status==="pendiente"?"border-blue-300":"border-gray-200"}`}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                {linkedOT&&<div className="mt-2 inline-flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs px-3 py-1 rounded-full"><CheckCircle size={10}/>OT: {linkedOT.code}</div>}
              </div>
              {user.role==="supervisor"&&r.status==="pendiente"&&(
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={()=>approve(r)} className="flex items-center gap-1.5 text-white text-xs px-3 py-1.5 rounded-lg font-medium" style={{background:NV.blue}}><Check size={12}/>Aprobar + OT</button>
                  <button onClick={()=>reject(r)} className="flex items-center gap-1.5 bg-red-50 border border-red-200 text-red-700 text-xs px-3 py-1.5 rounded-lg"><X size={12}/>Rechazar</button>
                </div>
              )}
            </div>
          </div>
